...
import { adminMemberEditHref, adminMembersHref } from "@/lib/admin-members-links";
...
function statusBadgeClass(status: MemberRosterStatus) {
  switch (status) {
    case "active":
...
export function AdminMembersDirectory({ initialMembers }: { initialMembers: AdminMemberRosterRow[] }) {
      ...
      <header className="shrink-0 px-4 pb-4 pt-[max(1rem,env(safe-area-inset-top,0px))] lg:px-8 lg:pb-5 lg:pt-5">
        <div className="mb-4 flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-lg font-bold text-white lg:text-xl">All members</h1>
            <p className="mt-0.5 text-sm text-white/55">
              Across all cell groups · {filtered.length} in this view
            </p>
          </div>
        </div>
          ...
          {filtered.length === 0 ? (
            <p className="py-10 text-center text-sm text-neutral-500">
              {initialMembers.length === 0
                ? "No members in the roster yet. Add people from the cell leader app once member records are stored in the database."
